...
class MisMatchedTokenizer:
    ...
    def encode(self, words: list, add_special_tokens=False):
        ...
        offsets = []
        #print("word:", words)
        #print("self.tokenizer_vocab:", self.tokenizer_vocab)
        for word in words:
            wordpieces = self.tokenizer.tokenize(word)
            #print("wordpieces:", wordpieces)
            wordpiece_ids = []
            for wordpiece in wordpieces:
                if wordpiece in self.tokenizer_vocab:
                    wordpiece_ids.append(self.tokenizer_vocab[wordpiece])
                else:
                    wordpiece_ids.append(self.tokenizer.unk_token_id)
