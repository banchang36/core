...
class Trainer:
    def __init__(self, args):
        ...
        print("batch_size:",self.train_loader.batch_size)
        ...
        self.model, self.optimizer =  self.setup_model_optimizer_and_scheduler(
                                                    model=model, 
                                                    config=config, 
                                                    num_steps_per_epoch=len(self.train_loader), 
                                                    warmup=args.warmup)
        self.lr_scheduler = StepLR(self.optimizer, step_size=10)
    ...
    def setup_model_optimizer_and_scheduler(self, model, config, num_steps_per_epoch, warmup):
        ...
        #lr_scheduler = self.init_scheduler(optimizer=optimizer, num_steps_per_epoch=num_steps_per_epoch, warmup_ratio=warmup)
        if self.model_dir and self.ckpt_id:
            model.load_state_dict(torch.load(os.path.join(self.model_dir, f"epoch-{self.ckpt_id}.pt")))
            print(f"load model from {self.model_dir}")
            for param_group in optimizer.param_groups:
                param_group['lr'] = self.lr
        else:
    ...
    def train(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            if self.do_eval:
                ...
                print(metrics)
            print(metrics["valid_loss"])
            if metrics["valid_loss"] <= metrics["best_valid_loss"]:
              #self._save_ckpt(epoch)
              self._save_ckpt("model")
              print("tot hon")

    def _save_ckpt(self, epoch):
        self.model.save_checkpoint(self.save_dir, f"{epoch}")
